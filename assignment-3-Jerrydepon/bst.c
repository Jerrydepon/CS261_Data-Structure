...
int _bst_subtree_size(struct bst_node* n) {
  ...
  if (n == NULL) {
    ...
    subtree_size = 1 + _bst_subtree_size(n->left) + _bst_subtree_size(n->right);
  }
...
int bst_size(struct bst* bst) {
  int size = 0;
  size = _bst_subtree_size(bst->root);
  return size;
}
...
int _bst_subtree_height(struct bst_node* n) {
  ...
  int left = _bst_subtree_height(n->left);
  int right = _bst_subtree_height(n->right);

  if (left > right) {
    subtree_height = left + 1;
    return subtree_height;
  } else {
    subtree_height = right + 1;
    return subtree_height;
  }
}
...
int bst_height(struct bst* bst) {
  int height = 0;
  height = _bst_subtree_height(bst->root);
  return height;
}
...
int _bst_subtree_path_sum(int sum, struct bst_node* n) {
  ...
  if (n->left) {
    equal_sum = equal_sum || _bst_subtree_path_sum(sum - n->val, n->left);
  }
  if (n->right) {
    equal_sum = equal_sum || _bst_subtree_path_sum(sum - n->val, n->right);
  }

  return equal_sum; //return 1 if sum equals path
}


int bst_path_sum(int sum, struct bst* bst) {
  int equal_sum = 0;
  equal_sum = _bst_subtree_path_sum(sum, bst->root);
  return equal_sum;
}
 ...
 */
void push_in_order(struct bst_node *n, struct bst_iterator *iter) {
  if (n == NULL) {
    return;
  }

  // push nodes to the stack if there is left child
  while (n) {
    stack_push(iter->s, n);
    n = n->left;
  }
...
int bst_iterator_next(struct bst_iterator* iter) {
  assert(iter);
  struct bst_node* n = stack_pop(iter->s); // remember the pop node
  
  push_in_order(n->right, iter); //push if there is a right child

  return n->val; // pass the pop node's value to print out
}
